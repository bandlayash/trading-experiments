...
| **MAG7 / Magnificent Seven** | The seven mega-cap US tech names — Apple, Microsoft, Alphabet, Amazon, Nvidia, Meta, Tesla — treated informally as a basket because of their outsized weight in US index returns. Used on the [`strategy/mag7-overnight`](../../tree/strategy/mag7-overnight) branch. |
...
| **MOC / MOO order** | Market-On-Close / Market-On-Open — an order that executes automatically at the closing or opening auction price rather than at a specified limit. The natural order type for an overnight (close-to-open) strategy. |
| **Closing / opening auction** | A batch process, distinct from continuous trading, that matches all orders submitted for it into one official open or close print. Most large institutional flow (index rebalancing, fund flows) executes here rather than in continuous trading. |

---

## Overnight / intraday effect terms

Used on the [`strategy/mag7-overnight`](../../tree/strategy/mag7-overnight) branch.

| Term | Definition |
|---|---|
| **Overnight return** | The return from one day's close to the next day's open — held while the market is closed, capturing after-hours news, earnings reactions, and the opening auction, but no continuous-session trading. |
| **Intraday / day-session return** | The complementary return, from a day's open to that same day's close — everything that happens while the market is actually trading. |
| **Close-to-open** | Another name for the overnight window; the period this strategy is exposed to the market. |
| **Overnight drift** | The empirical finding, documented across decades of US equity data, that most of the market's long-run gain has accrued overnight rather than intraday — see the `strategy/mag7-overnight` branch for citations and caveats. |
| **Dispersion (cross-sectional)** | How much individual names' returns differ from each other on a given day, as opposed to the market average. A strategy applied to a basket cares about dispersion as well as the average return. |
| **Gap risk** | The risk that price moves sharply between one session's close and the next session's open, with no opportunity to exit in between — a stop-loss cannot protect against it. Directly relevant to any position held overnight. |
| **Earnings reaction** | The price move following a company's quarterly results, which for most US companies are released after the market closes or before it opens — i.e. overnight, not during continuous trading. |
...
## Hidden Markov Model terms

Used on the [`strategy/hmm-regime`](../../tree/strategy/hmm-regime) branch.

| Term | Definition |
|---|---|
| **HMM** (Hidden Markov Model) | A model of a sequence generated by an unobserved ("hidden") state that changes over time, where each state produces observations drawn from its own distribution. Here, the hidden state is the market **regime**, and the observation is the daily return. |
| **State / regime** | One of a small number of latent conditions the model believes the market can be in — e.g. "trending" versus "choppy" — each with its own typical return and volatility. Never observed directly, only inferred. |
| **Latent variable** | A quantity a model reasons about but never directly sees. The regime is latent; only its downstream effect (the return) is observed. |
| **Emission distribution** | The probability distribution a state generates observations from. This repo uses a Gaussian (Normal) emission per state, i.e. each regime has its own mean and variance of daily returns. |
| **Transition matrix** | The probability of moving from each state to each other state (including staying put) from one day to the next. Encodes how "sticky" regimes are. |
| **Start (initial) probability** | The probability distribution over states at the very first observation, before any data has updated the belief. |
| **Filtering** | Estimating the probability of the CURRENT state using only past and present observations — the only estimate actually available to a live strategy, since it has never seen tomorrow. |
| **Smoothing** | Estimating the probability of a PAST state using the entire observed sequence, including observations that came after it. Standard for after-the-fact analysis, but a look-ahead leak if used to size a live position, because it uses the future. |
| **Forward algorithm** | The recursive procedure that computes the filtering distribution efficiently, one observation at a time, without recomputing from scratch. |
| **Forward-backward algorithm** | The combined recursion (forward pass plus a backward pass) that computes the smoothing distribution. `hmmlearn`'s `predict_proba` implements this — see the caveat on the `strategy/hmm-regime` branch about why it cannot be used naively for a live decision. |
| **Viterbi algorithm** | A related recursion that finds the single most likely STATE SEQUENCE, rather than a probability per state per day. Not used here; this strategy trades a continuous probability threshold instead of a hard sequence decode. |
| **Expectation-Maximization (EM)** | The iterative algorithm used to fit an HMM's parameters (transition matrix, emission means/variances) to data: alternately estimate the hidden states given current parameters, then re-estimate parameters given those states, until convergence. |
| **Label switching** | The fact that an HMM's state indices carry no fixed meaning across separate fits — state 0 in one fit might be the "bull" regime, and state 0 in the next fit might be the "bear" regime. Any strategy that refits must re-derive which state means what every time, rather than trusting the index. |
| **Gaussian HMM** | An HMM whose emission distributions are Gaussian (Normal) — the specific, common variant used here, via the `hmmlearn` Python library. |
| **Walk-forward refit** | Periodically re-fitting a model using only the data available up to that point in time, then using the newly fit model going forward until the next refit — as opposed to fitting once on an entire historical sample, which would let the model "see" the future relative to earlier points in the backtest. |
